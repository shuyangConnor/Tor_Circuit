...
from dependency.torpy.ext.utils import cached_property
...
from dependency.torpy.ext.utils import ignore
from dependency.torpy.ext.http.client import HttpStreamClient
from dependency.torpy.ext.stream import TorStream, TorWindow, StreamsList
from dependency.torpy.ext.crypto_state import CryptoState
from dependency.torpy.ext.keyagreement import KeyAgreement, TapKeyAgreement, NtorKeyAgreement, FastKeyAgreement
from dependency.torpy.ext.hiddenservice import DescriptorNotAvailable, HiddenServiceConnector
...
class CircuitNode:
    ...
    def store_key(self, shared_secret):
        self._crypto_state = CryptoState(shared_secret)
...
class TorCircuit:
    ...
    def create(self):
        with self._state_lock:
            ...
            self.circuit_nodes = self._initialize(self._guard.router)
    ...
    def _circuit_nodes(self):
        return self.circuit_nodes[2:]
    ...
    @property
    def associated_hs(self):
        return self._associated_hs

    @associated_hs.setter
    def associated_hs(self, hs):
        self._associated_hs = hs
    ...
    def build_hops(self, hops_count):
        ...
        if hops_count == 3:
            middle_router = self._guard.consensus.get_random_middle_node()
            if self._guard.router.or_port == middle_router.or_port:
                middle_router = self._guard.consensus.get_random_middle_node()
            if self._guard.router.or_port == middle_router.or_port:
                middle_router = self._guard.consensus.get_random_middle_node()
            if self._guard.router.or_port == middle_router.or_port:
            ...
            exit_router = self._guard.consensus.get_random_exit_node()
            if self._guard.router.or_port == exit_router.or_port or exit_router.or_port == middle_router.or_port:
                exit_router = self._guard.consensus.get_random_exit_node()
            if self._guard.router.or_port == exit_router.or_port or exit_router.or_port == middle_router.or_port:
                exit_router = self._guard.consensus.get_random_exit_node()
            if self._guard.router.or_port == exit_router.or_port or exit_router.or_port == middle_router.or_port:
            ...
            print(self._guard.router.or_port, middle_router.or_port, exit_router.or_port)
            self.extend(middle_router)
            self.extend(exit_router)
        else:
    ...
    def send_relay_cell(self, inner_cell, expected_response_type):
        cell_established = self.send_relay_wait(inner_cell, expected_response_type)
        # tor_ref: hs_client_receive_rendezvous_acked
        return cell_established
    ...
    def send_relay_cell(self, inner_cell, expected_response_type):
        ...
        cell_ack = self.send_relay_wait(inner_cell, expected_response_type)
        return cell_ack

    @check_connected
    def extend_to_hidden(self, _hidden_service):
        # logger.info('Extending #%x circuit for hidden service %s...', self.id, hidden_service.hostname)
        import hidden_service_COMPLETED
        self._associated_hs = _hidden_service
        hidden_service_COMPLETED.extend_to_hidden(self, _hidden_service)
